...
func (h *handler) proxyPackument(w http.ResponseWriter, r *http.Request, up upstream, projectID, project, repo, pkg string) {
	body, err := h.upstream.fetchPackument(r.Context(), up, pkg)
	if err != nil {
		if errors.Is(err, errUpstreamNotFound) {
			// Nothing upstream; the local cache is the last word.
			h.getLocalPackument(w, r, projectID, project, repo, pkg, http.StatusNotFound)
			return
		}
		h.log.Warn("npm upstream packument unreachable; trying cache",
			slog.String("pkg", pkg), slog.String("error", err.Error()))
		h.getLocalPackument(w, r, projectID, project, repo, pkg, http.StatusBadGateway)
		return
	}

	rewritten, err := rewritePackument(body, registryBase(r, project, repo), pkg)
	if err != nil {
...
func (h *handler) getLocalPackument(w http.ResponseWriter, r *http.Request, projectID, project, repo, pkg string, notFoundStatus int) {
...
func (h *handler) proxyTarball(w http.ResponseWriter, r *http.Request, up upstream, projectID, repo, pkg, filename string) {
	...
	if digest, size, err := h.store.tarball(r.Context(), projectID, repo, pkg, version); err == nil {
	...
	digest, size, err := h.cacheTarball(r.Context(), up, projectID, repo, pkg, version, filename)
	if err != nil {
		if errors.Is(err, errUpstreamNotFound) {
			writeError(w, h.log, http.StatusNotFound, "not found")
			return
...
func (h *handler) cacheTarball(ctx context.Context, up upstream, projectID, repo, pkg, version, filename string) (string, int64, error) {
