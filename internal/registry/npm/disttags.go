...
func (h *handler) serveDistTags(w http.ResponseWriter, r *http.Request, repo repository.Repository, op npmOp) {
	switch r.Method {
	case http.MethodGet:
		h.listDistTags(w, r, repo.ID, op.pkg)
	case http.MethodPut:
		h.setDistTag(w, r, repo, op.pkg, op.ref)
	case http.MethodDelete:
		h.deleteDistTag(w, r, repo, op.pkg, op.ref)
	default:
		writeError(w, h.log, http.StatusMethodNotAllowed, "method not allowed")
	}
}

func (h *handler) listDistTags(w http.ResponseWriter, r *http.Request, repositoryID, pkg string) {
	tags, err := h.store.distTags(r.Context(), repositoryID, pkg)
	if err != nil {
...
func (h *handler) setDistTag(w http.ResponseWriter, r *http.Request, repo repository.Repository, pkg, tag string) {
	if tag == "" {
	...
	if err := h.store.setDistTag(r.Context(), repo.ID, repo.ProjectID, pkg, tag, version, actorFrom(r)); err != nil {
...
func (h *handler) deleteDistTag(w http.ResponseWriter, r *http.Request, repo repository.Repository, pkg, tag string) {
	if tag == "" {
		writeError(w, h.log, http.StatusBadRequest, "missing dist-tag")
		return
	}
	...
	if err := h.store.deleteDistTag(r.Context(), repo.ID, repo.ProjectID, pkg, tag, actorFrom(r)); err != nil {
		if errors.Is(err, ErrPackageNotFound) {
