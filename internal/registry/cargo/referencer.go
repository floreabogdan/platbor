...
// Referencer reports the blob digests Cargo .crate files still need, so garbage
...
// NewReferencer builds a Cargo blob referencer over the metadata database.
func NewReferencer(sqlDB *sql.DB) *Referencer {
	return &Referencer{q: db.New(sqlDB)}
}

// ReferencedBlobs returns the set of digests every stored/cached .crate points
...
func (r *Referencer) ReferencedBlobs(ctx context.Context) (map[string]struct{}, error) {
	digests, err := r.q.ListCargoBlobDigests(ctx)
	if err != nil {
		return nil, fmt.Errorf("listing cargo blob digests: %w", err)
	}
