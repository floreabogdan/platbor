...
- [ ] Repo scaffolding: Go module, `cmd/platbor`, config loading (file + env), CI (lint, test, build)
- [ ] SQLite + migrations + sqlc wiring; Postgres behind the same interface
...
- [ ] React app shell: login, sidebar layout, projects list (per DESIGN-SYSTEM.md), embedded via `go:embed`
