...
- [x] Webhooks (project event subscriptions) and per-project storage quotas. **Webhooks** ride the audit log (the event stream), so no adapter emits events: a background dispatcher tails audit entries from a persisted cursor and POSTs signed (`X-Platbor-Signature` HMAC-SHA256) JSON deliveries to a project's endpoints, filtered by action prefix. **Quotas** (`quota_bytes`, 0 = unlimited) enforce at the shared write chokepoint (`repository.ResolveOrCreate`) across all formats, with usage summed across every format (incl. OCI manifest/layer bytes) and shown in the UI. Both managed under a project (create/list/delete webhooks; set quota / view usage), admin-only.
